...
class PhaseRetrieval():
    ...
    def align(self,unaligned, ref, return_meta=False):
        ...
        shif = shift.astype(np.int)
        aligned = np.roll(np.rot90(unaligned,k=n_rot),(shif[0],shif[1]),axis=(0,1))
        if return_meta:
    ...
    def calc_real_space_error(self, true_im, plot=True):
        ...
        if plot:           
            ...
            plt.imshow(self.real_space_guess)
            plt.ylabel('$y$')
            plt.xlabel('$x$')
                        
            plot = fig.add_subplot(1,3,3)
            plt.title('Reconstruction (aligned)')
            plt.imshow(fixed)
            plt.ylabel('$y$')
            plt.xlabel('$x$')
            plt.show()
            ...
            fig.suptitle('Phase Retrieval of a 2D image')
            plt.show()
